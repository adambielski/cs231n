  ...
  def loss(self, X, y=None):
    ...
    for i in range(N_layers-1):
      if self.use_batchnorm:
        ...
        out, cache = affine_batchnorm_relu_forward(out, self.params[wstr], self.params[bstr], self.params[gammastr], self.params[betastr], self.bn_params[i])
        caches.append(cache)
      else:
      ...
      if self.use_dropout:
        ...
        droput_caches.append(cache)
    out, cache = affine_forward(out, self.params['W' + str(N_layers)], self.params['b' + str(N_layers)])
    scores = out
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    ############################################################################
    loss, dout = softmax_loss(scores, y)
    reg_loss = sum([np.sum(np.square(self.params['W' + str(i)])) for i in range(1, N_layers)])
    loss += (0.5 * self.reg * reg_loss)

    dx, dw, db = affine_backward(dout, cache)
    dwstr, dbstr, dgammastr, dbetastr = get_params_name(N_layers)
    grads[dwstr] = dw
    grads[dbstr] = db
    for i in reversed(range(N_layers-1)):
      dwstr, dbstr, dgammastr, dbetastr = get_params_name(i+1)
      if self.use_dropout:
        dx = dropout_backward(dx, droput_caches[i])
      if self.use_batchnorm:
        dx, dw, db, dgamma, dbeta = affine_batchnorm_relu_backward(dx, caches[i])
        grads[dbetastr] = dbeta
        grads[dgammastr] = dgamma
      else:
        dx, dw, db = affine_relu_backward(dx, caches[i])
      dw += self.reg * self.params[dwstr]
      grads[dwstr] = dw
      grads[dbstr] = db
    ############################################################################
    #                             END OF YOUR CODE                             #
